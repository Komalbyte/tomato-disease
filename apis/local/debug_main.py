...
async def predict(file: UploadFile = File(...)):
    ...
    # Create a hash of the image to track different images
    image_hash = hashlib.md5(file_bytes).hexdigest()[:8]
    print(f"\n--- Processing image with hash: {image_hash} ---")
    ...
    print(f"Image array shape: {img_array.shape}, min: {img_array.min():.3f}, max: {img_array.max():.3f}")
    ...
    if model is not None:
        try:
            ...
            pred = predictions[0]
            print(f"Raw prediction values: {pred}")
            
            # Show all class probabilities
            print("All class probabilities:")
            for i, (class_name, prob) in enumerate(zip(class_names, pred)):
    ...
    print(f"--- End processing image {image_hash} ---\n")
    
    return {
        'pred_class': pred_class,
        'pred_conf': float(pred_conf),
        'all_probabilities': {class_name: float(prob) for class_name, prob in zip(class_names, pred)}
    }
